...
M.default_opts = {
    ...
    surrounds = {
        ...
        ["i"] = { -- TODO: Add find/delete/change functions
            add = function()
                local left_delimiter = input.get_input(
        ...
        ["t"] = {
            ...
            change = {
                ...
                replacement = function()
                    ...
                    if user_input then
                        local element = user_input:match("^<?([^%s>]*)")
                        local attributes = user_input:match(
                                               "^<?[^%s>]*%s+(.-)>?$")
    ...
    indent_lines = function(start, stop)
        ...
        if start <= stop and
            (b.equalprg ~= "" or b.indentexpr ~= "" or b.cindent or
                b.smartindent or b.lisp) then
...
---@nodiscard
M.get_input = function(prompt) return input.get_input(prompt) end
...
M.get_selection = function(args)
    ...
    elseif args.query then
        return require("nvim-surround.queries").get_selection(
                   args.query.capture, args.query.type)
    elseif args.textobject then ---@diagnostic disable-line: undefined-field
        vim.deprecate("The `textobject` key for `config.get_selection`",
                      "`motion`", "v2.0.0", "nvim-surround")
    else
        vim.notify(
...
M.get_selections = function(args)
    ...
    if args.pattern then
        return require("nvim-surround.patterns").get_selections(selection,
                                                                args.pattern)
    elseif args.exclude then
        local outer_selection = M.get_opts().surrounds[args.char].find()
        if not outer_selection then return nil end
        vim.fn.cursor(outer_selection.first_pos)
        local inner_selection = args.exclude()
        if not inner_selection then return nil end
        -- Properly exclude the inner selection from the outer selection
        local selections = {
            left = {
                first_pos = outer_selection.first_pos,
                last_pos = {
                    inner_selection.first_pos[1],
                    inner_selection.first_pos[2] - 1
                }
            },
            right = {
                first_pos = {
                    inner_selection.last_pos[1], inner_selection.last_pos[2] + 1
                },
...
M.get_alias = function(char)
    ...
    if type(aliases[char]) == "string" then return aliases[char] end
    return char
end
...
M.get_change = function(char)
    ...
    if M.get_opts().surrounds[char] then
        if M.get_opts().surrounds[char].change then
            ...
            return {target = M.get_delete(char)}
        end
    end
...
M.translate_add = function(user_add)
    if not user_add then return false end
    -- If the add key is already in internal form, return
    if type(user_add) == "function" then return user_add end
    -- Otherwise, wrap the string tables in a function
    return function()
        return {
            functional.to_list(user_add[1]), functional.to_list(user_add[2])
        }
    end
end
...
M.translate_find = function(user_find)
    if not user_find then return false end
    if type(user_find) == "function" then return user_find end
    -- Treat the string as a Lua pattern, and find the selection
    return function() return M.get_selection({pattern = user_find}) end
end
...
M.translate_delete = function(char, user_delete)
    if not user_delete then return false end
    if type(user_delete) == "function" then return user_delete end
    -- Treat the string as a Lua pattern, and find the selection
    return function()
...
M.translate_change = function(char, user_change)
    if user_change == nil then return nil end
    if user_change == false then return false end
    return {
...
M.translate_surround = function(char, user_surround)
    if not user_surround then return false end
    return {
...
M.translate_invalid_key_behavior = function(invalid_surround)
    ...
    local invalid = M.translate_surround("invalid_key_behavior",
                                         invalid_surround)
    if invalid == false then return noop_surround end
    if invalid.add == false then invalid.add = noop_surround.add end
    if invalid.find == false then invalid.find = noop_surround.find end
    if invalid.delete == false then invalid.delete = noop_surround.delete end
...
M.translate_opts = function(user_opts)
    if user_opts.highlight_motion then ---@diagnostic disable-line: undefined-field
        vim.deprecate("`config.highlight_motion`", "`config.highlight`",
                      "v2.0.0", "nvim-surround")
    end
    if user_opts.delimiters then ---@diagnostic disable-line: undefined-field
        vim.deprecate("`config.delimiters`", "`config.surrounds`", "v2.0.0",
                      "nvim-surround")
    end
    ...
    if not user_opts.surrounds then return opts end
    ...
    for char, user_surround in pairs(user_opts.surrounds) do
        if char == "pairs" or char == "separators" then
            vim.deprecate(
                ...
                "`config.surrounds`", "v2.0.0", "nvim-surround")
        end
        ...
        if type(user_surround) ~= "nil" then
            if char == "invalid_key_behavior" then
                opts.surrounds[char] = M.translate_invalid_key_behavior(
                                           user_surround)
            else
...
M.set_keymap = function(args)
    ...
    if not args.lhs then
        ...
        if not M.user_opts.keymaps[args.name] then return end
...
M.set_keymaps = function(args)
    ...
    M.set_keymap({
        ...
        rhs = function()
            return "^" .. tostring(vim.v.count1) ..
                       "<Plug>(nvim-surround-normal)g_"
        end,
        opts = {
            buffer = args.buffer,
    ...
    M.set_keymap({
        ...
        rhs = function()
            return "^" .. tostring(vim.v.count1) ..
                       "<Plug>(nvim-surround-normal-line)g_"
        end,
        opts = {
            buffer = args.buffer,
    ...
    M.set_keymap({
        ...
        opts = {desc = "Add a surrounding pair around the cursor (insert mode)"}
    })
    M.set_keymap({
        name = "insert_line",
        mode = "i",
    ...
    M.set_keymap({
        ...
        opts = {desc = "Add a surrounding pair around a motion (normal mode)"}
    })
    M.set_keymap({
        name = "normal_cur",
        mode = "n",
    ...
    M.set_keymap({
        ...
        opts = {desc = "Add a surrounding pair around a visual selection"}
    })
    M.set_keymap({
        name = "visual_line",
        mode = "x",
    ...
    M.set_keymap({
        ...
        opts = {desc = "Delete a surrounding pair"}
    })
    M.set_keymap({
        name = "change",
        mode = "n",
